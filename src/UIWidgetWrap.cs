...
public class UIWidgetWrap
{
	...
	public static void Register(IntPtr L)
	{
		...
		{
			new LuaMethod("ShaderToGray", new LuaCSFunction(UIWidgetWrap.ShaderToGray)),
			new LuaMethod("ShaderToNormal", new LuaCSFunction(UIWidgetWrap.ShaderToNormal)),
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("localCenter", new LuaCSFunction(UIWidgetWrap.get_localCenter), null),
			new LuaField("worldCorners", new LuaCSFunction(UIWidgetWrap.get_worldCorners), null),
			new LuaField("worldCenter", new LuaCSFunction(UIWidgetWrap.get_worldCenter), null),
			...
			new LuaField("border", new LuaCSFunction(UIWidgetWrap.get_border), new LuaCSFunction(UIWidgetWrap.set_border))
		};
	...
	private static int get_localCenter(IntPtr L)
	{
		...
		LuaScriptMgr.Push(L, uIWidget.localCenter);
		return 1;
	}
	...
	private static int get_worldCenter(IntPtr L)
	{
		...
		LuaScriptMgr.Push(L, uIWidget.worldCenter);
		return 1;
	}
	...
	private static int set_border(IntPtr L)
	{
		...
		uIWidget.border = LuaScriptMgr.GetVector4(L, 3);
		return 0;
	}

	[MonoPInvokeCallback(typeof(LuaCSFunction))]
	private static int ShaderToGray(IntPtr L)
	{
		LuaScriptMgr.CheckArgsCount(L, 1);
		UIWidget uIWidget = (UIWidget)LuaScriptMgr.GetUnityObjectSelf(L, 1, "UIWidget");
		uIWidget.ShaderToGray();
		return 0;
	}

	[MonoPInvokeCallback(typeof(LuaCSFunction))]
	private static int ShaderToNormal(IntPtr L)
	{
		LuaScriptMgr.CheckArgsCount(L, 1);
		UIWidget uIWidget = (UIWidget)LuaScriptMgr.GetUnityObjectSelf(L, 1, "UIWidget");
		uIWidget.ShaderToNormal();
		return 0;
	}
	...
	private static int WriteToBuffers(IntPtr L)
	{
		...
		UIWidget uIWidget = (UIWidget)LuaScriptMgr.GetUnityObjectSelf(L, 1, "UIWidget");
		BetterList<Vector3> v = (BetterList<Vector3>)LuaScriptMgr.GetNetObject(L, 2, typeof(BetterList<Vector3>));
		BetterList<Vector2> u = (BetterList<Vector2>)LuaScriptMgr.GetNetObject(L, 3, typeof(BetterList<Vector2>));
		BetterList<Color32> c = (BetterList<Color32>)LuaScriptMgr.GetNetObject(L, 4, typeof(BetterList<Color32>));
